...
-- SQL --> case sensitive değildir. (büyük-küçük harf ayrımı gözetmez)
...
SELECT	name as ismi, Composer as besteci
FROM	tracks
...
--Berlin' de 2 dolardan yüksek tutara sahip invoice' ların en yüksek 3 tanesini getiriniz?
SELECT *
FROM 	invoices
...
/* -- database'de kaç farklı composer var? 
...
SELECT	min(Milliseconds) AS enkısa, max(milliseconds) AS enuzun
FROM	tracks;
...
-- ablumıd ye göre parça isimleri grupla
SELECT	name, count(name), AlbumId
FROM	tracks
GROUP BY AlbumId;


--- herbir yazarın(composer) toplam track sayısını göster
